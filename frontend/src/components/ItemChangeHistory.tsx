...
const ItemChangeHistory: React.FC<ItemChangeHistoryProps> = ({
  changelog,
  loading,
  error,
}) => {
  ...
  return (
    <div className="space-y-4">
      {changelog.map((log) => {
        const presentation =
          ACTION_PRESENTATION[log.action] ?? DEFAULT_ACTION_PRESENTATION;
        return (
          <div
            key={log.id}
            className={`rounded-xl border p-4 transition-all ${presentation.color}`}
          >
