...
const EditItemForm: React.FC<EditItemFormProps> = ({
  ...
  const normalisePayload = (values: ItemFormSchema): ItemPayload => {
    ...
    return {
      ...
      purchase_date:
        purchaseDate && purchaseDate.length > 0 ? purchaseDate : null,
      value,
      location:
        typeof values.location === "number" && !Number.isNaN(values.location)
          ? values.location
          : null,
  ...
  const onSubmit = async (values: ItemFormSchema) => {
    ...
    try {
      ...
      setFormError(
        "Der Gegenstand konnte nicht aktualisiert werden. Bitte versuche es erneut.",
      );
    }
  };
  ...
  return (
    <form
      className="flex h-full flex-col text-slate-700"
    ...
    >
      <div className="flex-grow overflow-y-auto px-3 py-3 sm:px-5 sm:py-5">
        <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
          ...
          <div className="space-y-3">
            <label
              htmlFor="wodis_inventory_number"
              className="text-sm font-medium text-slate-800"
            >
            ...
            <div className="flex flex-col gap-1 text-xs">
              {errors.wodis_inventory_number && (
                <p className="text-red-500">
                  {errors.wodis_inventory_number.message}
                </p>
              )}
