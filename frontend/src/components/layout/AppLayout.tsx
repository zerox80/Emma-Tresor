...
const AppLayout: React.FC = () => {
  ...
  const pageMeta = useMemo(() => {
    ...
    return {
      title: "Dashboard",
      subtitle:
        "Willkommen zurück! Hier findest du alle Kennzahlen, schnelle Aktionen " +
        "und einen klaren Startpunkt mit EmmaTresor.",
    };
  ...
  return (
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-[-12rem] flex justify-center blur-3xl">
        <div
          className={[
            "aspect-[3/2] w-[60rem] rounded-full bg-gradient-to-br from-brand-200/60",
            "via-sky-100/50 to-white opacity-70",
          ].join(" ")}
        />
      </div>
      ...
      <header className="border-b border-slate-200 bg-white/90 backdrop-blur-xl">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <Link
            to="/"
            className="flex items-center gap-3 text-lg font-semibold text-brand-600"
          >
            {/* Logo icon */}
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-100 text-brand-700 shadow-sm">
              E
            </span>
            {/* Brand text */}
            EmmaTresor Inventar
          ...
          <button
            type="button"
            className={[
              "inline-flex items-center justify-center rounded-lg border border-slate-200 px-3",
              "py-2 text-xs font-semibold uppercase tracking-wide text-slate-600 transition",
              "hover:border-brand-200 hover:text-brand-600 md:hidden",
            ].join(" ")}
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={[
                "rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold uppercase",
                "tracking-wide text-slate-600 transition hover:border-red-300 hover:text-red-500",
              ].join(" ")}
            >
              Abmelden
            </button>
      ...
      <main className="relative z-10 mx-auto flex max-w-7xl flex-col gap-6 px-6 py-8">
        ...
        <div
          className={[
            "rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-lg",
            "shadow-slate-200/60 backdrop-blur-xl",
          ].join(" ")}
        >
          {/* Dynamic page title */}
