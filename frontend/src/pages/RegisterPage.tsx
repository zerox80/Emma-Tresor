...
const registerSchema = z
  .object({
    ...
    acceptTerms: z
      .boolean()
      .refine(Boolean, "Bitte bestätige die Nutzungsbedingungen"),
  })
  ...
  const onSubmit = async (values: RegisterSchema) => {
    ...
    try {
      ...
      const axiosError = error as AxiosError<
        RegisterErrorResponse | string | string[]
      >;
      ...
      setFormError(
        detailedMessage ??
          "Die Registrierung konnte nicht abgeschlossen werden. Bitte überprüfe " +
            "deine Angaben oder versuche es in wenigen Augenblicken erneut.",
      );
    }
  };
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          className={[
            "w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-slate-800",
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-slate-500">
          Mindestens 12 Zeichen, inkl. Groß- und Kleinbuchstaben, Ziffern und
          Sonderzeichen.
        </p>
      </div>
      ...
      <label className="flex items-start gap-3 text-xs text-slate-600">
        <input
          type="checkbox"
          className={[
            "mt-1 h-4 w-4 rounded border border-slate-300 bg-white text-brand-500",
            "focus:outline-none focus:ring-2 focus:ring-brand-200/60",
          ].join(" ")}
          {...register("acceptTerms")}
        />
        <span>
          Ich bestätige die Sicherheits- und Nutzungsbedingungen von EmmaTresor.
          Meine Daten bleiben in meiner Kontrolle und werden nicht an Dritte
          weitergegeben.
        </span>
      </label>
      {errors.acceptTerms && (
