...
export const fetchItemByAssetTag = async (assetTag: string): Promise<Item> => {
  ...
  const { data } = await apiClient.get<Item>(
    `/items/lookup_by_tag/${encodedTag}/`,
  );
  return data; // Return the item data
};
 ...
 */
export const fetchItemQrCode = async (
  itemId: number,
  options: FetchItemQrCodeOptions = {},
): Promise<Blob> => {
  // Set download parameter if requested
  const params = options.download ? { download: "1" } : undefined;
  const { data } = await apiClient.get<Blob>(
    `/items/${itemId}/generate_qr_code/`,
    {
...
export interface ExportItemsOptions
  extends Omit<FetchItemsOptions, "page" | "pageSize"> {}
...
export const updateListItems = async (
  ...
  const { data } = await apiClient.patch<ItemList>(`/lists/${listId}/`, {
    items: itemIds,
  }); // PATCH with new item IDs
 ...
 */
export const fetchItemChangelog = async (
  itemId: number,
): Promise<ItemChangeLog[]> => {
  const { data } = await apiClient.get<ItemChangeLog[]>(
    `/items/${itemId}/changelog/`,
  ); // GET request to changelog endpoint
...
export * from "./inventoryDuplicates";
